...
export function App() {
  ...
  useEffect(() => {
    if (timer) {
      ...
      const total_duration = timer.Config.Duration[timer.Mode]
      const progress = ((total_duration - timer.Duration) / total_duration) * 100
  ...
  if (timer) {
    ...
    return (
      <div class="min-h-screen flex flex-col justify-center items-center bg-zinc-300 text-zinc-900 dark:text-white dark:bg-zinc-900">
        <a href="" class="absolute top-4 right-4 p-2 rounded dark:bg-zinc-800 bg-white shadow-md">
        ...
        <div class="w-60 text-center flex flex-col gap-4">
          <div class="dark:bg-zinc-800 bg-white rounded-lg p-4 shadow-md flex gap-4 flex-col">
            <div>
              {mode}
            </div>
            ...
            <RemainingTimeCircle timer={timer} />
...
function RemainingTimeCircle(props) {
  return (
